...
const reverseCode = (longitude, latitude) => {
  return new Promise((resolve, reject) => {
    ...
    common_vendor.index.request({
      ...
      success(res) {
        common_vendor.index.__f__("log", "at utils/geocode.ts:29", "高德api接口返回", res);
        const data = res.data;
        if (data.status !== "1") {
          reject("高德api错误");
          return;
        ...
        common_vendor.index.__f__("log", "at utils/geocode.ts:36", "地址", address);
        resolve(address);
      },
      fail(err) {
...
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/geocode.js.map
